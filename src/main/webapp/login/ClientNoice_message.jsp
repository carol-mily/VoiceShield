...
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#demo'
            ,url: 'http://localhost:8080/test_war_exploded/ClientNoice_Controller.GetClientNoiceList'
            ,cols: [[
                {field:'client_usernoice_noiseID', title: '噪声ID', width:'20%', sort: true, }
                ,{field:'client_usernoice_noisePath', title: '噪声路径', width:'20%', sort: true}
                ,{field:'client_usernoice_ID', title: '用户ID', width:'20%', sort: true,}
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'edit'){
                ...
                var url = data.client_usernoice_noisePath;
                try {
                    var player = new Audio(url);
                    player.play()
                    //player.pause()
                }catch (e) {
                    Console.log("出错或者路径有问题")
                }
