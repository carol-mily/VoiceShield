...
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#demo'
            ,url: 'http://localhost:8080/test_war_exploded/Admit_Controller.GetUserlist'
            ,cols: [[
                {field:'username', title: '用户名', width:'10%', sort: true, fixed: true}
                ,{field:'password', title: '密码', width:'20%', sort: true}
                ,{field:'sex', title: '性别', width:'10%', sort: true}
                ,{field:'phonenum', title: '电话', width:'10%'}
                ,{field:'userType',title:'用户 类型',width:'10%'}
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                ...
                layer.confirm('真的删除行么', function(index){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'http://localhost:8080/test_war_exploded/Admit_Controller.DeleteUser',//发送请求
                ...
                var open_index=layer.open({
                    ...
                    maxmin: true,
                    area: ['70%', '70%'],
                    content: 'AdministratorChange.jsp',
                    btn:['关闭'],
                    btnAlign: 'c',
                    ...
                    success: function(layero, index){
                        ...
                        for (var i = 0; i < inputList.length; i++ ) {
                            if(i>=1){$(inputList[i+1]).val(editList[i]); continue;}
            ...
            switch(obj.event){
                case 'add':
                    ...
                    var open_index=layer.open({
                        ...
                        maxmin: true,
                        area: ['70%', '70%'],
                        content: 'AdministratorAdd.jsp',//设置你要弹出的jsp页面
                        btn:['关闭'],
                        btnAlign: 'c',
                        yes: function(index, layero){
                            //layer.msg("1的回调");
